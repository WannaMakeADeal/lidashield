...
def extract_phone_numbers_from_text(text):
    ...
    for match in PHONE_PATTERN.findall(text or ""):
        cleaned = re.sub(r"\s+", " ", match.strip()).strip().rstrip(".,);!?\'\"")
        digits = re.sub(r"\D", "", cleaned)
        if len(digits) >= 7 and cleaned not in found:
            found.append(cleaned)
    return found[:10]
...
def analyze_scam_message(message):
    """
    LidaShield AI Analyst Core v1.
    Database/graph-backed message analysis. Wording signals are treated as supporting evidence, not the main truth source.
    """
    text = message.strip()
    ...
    extracted_phones = extract_phone_numbers_from_text(text)
    extracted_domains = []

    evidence = []
    database_hits = []
    reasons = []
    max_database_score = 0
    behavior_score = 0
    verified_danger = False
    ...
    for url in extracted_urls:
        ...
        if domain in SHORTENER_DOMAINS:
            behavior_score += 10
            evidence.append({"type": "link_shortener", "indicator": domain, "confidence": 25, "detail": "Message uses a shortened link."})
        ...
        try:
            ...
            if known:
                ...
                evidence.append({"type": "verified_database", "indicator": normalized, "confidence": 95, "detail": f"URL found in LidaShield verified database via {known.get('source') or 'database'}."})
                max_database_score = max(max_database_score, 95 if known.get("verdict") == "dangerous" else 70)
                if known.get("verdict") == "dangerous":
                    verified_danger = True

            intel = lookup_intelligence_layer(normalized)
            if intel:
                intel_score = int(intel.get("score") or 0)
                max_database_score = max(max_database_score, intel_score)
                hit = {
                    "indicator_type": "url",
                    "url": url,
                    "normalized_url": normalized,
                    ...
                    "score": intel_score,
                    "report_count": intel.get("report_count", 0),
                    "event_count": intel.get("event_count", 0)
                }
                database_hits.append(hit)
                evidence.append({
                    "type": "intelligence_graph",
                    "indicator": normalized,
                    "confidence": intel_score,
                    "detail": f"URL matched LidaShield intelligence graph: {intel.get('source')} with score {intel_score}/100."
                })
                for reason in intel.get("reasons", [])[:5]:
                    evidence.append({"type": "intelligence_reason", "indicator": normalized, "confidence": min(90, intel_score), "detail": reason})
        except Exception:
            evidence.append({"type": "url_parse_error", "indicator": url, "confidence": 5, "detail": "URL could not be normalized for database lookup."})
    ...
    if extracted_urls:
        behavior_score += 8
        evidence.append({"type": "message_contains_url", "indicator": ", ".join(extracted_urls[:3]), "confidence": 15, "detail": "Message contains one or more URLs."})

    if extracted_emails:
        evidence.append({"type": "email_indicator", "indicator": ", ".join(extracted_emails[:3]), "confidence": 10, "detail": "Message contains email indicator(s)."})

    if extracted_phones:
        evidence.append({"type": "phone_indicator", "indicator": ", ".join(extracted_phones[:3]), "confidence": 12, "detail": "Message contains phone number indicator(s)."})

    if urgency_hits:
        behavior_score += 10
        evidence.append({"type": "behavioral_signal", "indicator": ", ".join(urgency_hits[:4]), "confidence": 18, "detail": "Uses urgency or pressure language."})

    if credential_hits:
        behavior_score += 14
        evidence.append({"type": "credential_request_signal", "indicator": ", ".join(credential_hits[:4]), "confidence": 24, "detail": "Mentions login, OTP, password, PIN, or account verification."})

    if bank_gov_hits:
        behavior_score += 10
        evidence.append({"type": "brand_or_institution_signal", "indicator": ", ".join(bank_gov_hits[:4]), "confidence": 18, "detail": "Mentions a bank, government service, or official institution."})

    if money_hits:
        behavior_score += 8
        evidence.append({"type": "payment_or_reward_signal", "indicator": ", ".join(money_hits[:4]), "confidence": 16, "detail": "Mentions money, refunds, prizes, fees, loans, parcels, crypto, or transfers."})

    if extracted_urls and len(text) < 90:
        behavior_score += 6
        evidence.append({"type": "short_link_message", "indicator": "message_length", "confidence": 10, "detail": "Short message with a link; common in phishing attempts."})

    behavior_score = min(40, behavior_score)  # behavior-only evidence cannot dominate the database.
    final_score = max(max_database_score, behavior_score)

    if verified_danger:
        verdict = "dangerous"
        analyst_summary = "Verified threat database match found. Treat this message as dangerous."
    elif max_database_score >= 65:
        verdict = "suspicious"
        analyst_summary = "LidaShield found high-risk intelligence evidence connected to this message."
    elif max_database_score >= 25:
        verdict = "unknown"
        analyst_summary = "LidaShield found watchlist intelligence, but not enough evidence for a suspicious verdict."
    elif behavior_score >= 30:
        verdict = "unknown"
        analyst_summary = "No database match found, but the message has scam-like behavioural signals."
    elif behavior_score > 0:
        verdict = "unknown"
        analyst_summary = "No database match found. Only weak behavioural signals were observed."
    else:
        verdict = "safe"
        analyst_summary = "No major database or behavioural scam signals were detected. This is not a safety guarantee."

    # User-facing reasons: evidence details, strongest first.
    evidence_sorted = sorted(evidence, key=lambda x: int(x.get("confidence") or 0), reverse=True)
    reasons = [item.get("detail") for item in evidence_sorted if item.get("detail")]
    if not reasons:
        reasons = [analyst_summary]
    ...
    return {
        ...
        "evidence": evidence_sorted[:20],
        "analyst_summary": analyst_summary,
        "signal_count": max(0, len(evidence_sorted)),
        "analysis_mode": "database_backed_ai_analyst_core"
    }
        ...
        with conn.cursor() as cur:
            ...
            cur.execute(
                ...
                (
                    ...
                    analysis.get("verdict"),
                    analysis.get("score", 0),
                    json.dumps(analysis.get("extracted_indicators", {})),
